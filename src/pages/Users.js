    ...
    <div>
      <div className="card">
        ...
        <div className="form-row" style={{marginTop:8}}>
          ...
          <button className="btn" onClick={()=>setSortAsc(s=>!s)}>{sortAsc ? 'Sort A→Z' : 'Sort Z→A'}</button>
        </div>
      </div>

      <div className="card">
        {loading && <div className="small">Loading users...</div>}
        {!loading && users.length===0 && <div className="small">No users</div>}
        {!loading && users.length>0 && <>
          <table className="table">
            <thead><tr><th>Name</th><th>Email</th><th>Phone</th><th>Company</th><th>Status</th></tr></thead>
            <tbody>
              {pageData.map(u => (
                <tr key={u.id} onClick={()=>gotoUser(u.id)} style={{cursor:'pointer'}}>
                  <td>{u.name}</td>
                  <td>{u.email}</td>
                  <td>{u.phone}</td>
                  <td>{u.company}</td>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:12}}>
            ...
            <div>
              ...
              <button className="btn" onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={page===totalPages} style={{marginLeft:8}}>Next</button>
            </div>
          </div>
        </>}
      </div>
